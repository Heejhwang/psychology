...
parameters {
  ...
  vector[N] slope1_raw;     real mu_slope1;         real<lower=0> sigma_slope1;
  vector[N] slope2_raw;     real mu_slope2;         real<lower=0> sigma_slope2;  
}
...
transformed parameters {
  ...
  vector[N] slope1 = mu_slope1 + sigma_slope1 * slope1_raw;  
  vector[N] slope2 = mu_slope2 + sigma_slope2 * slope2_raw;
...
model {
  ...
  mu_slope1 ~ normal(0.1, 0.05);
  sigma_slope1 ~ normal(0.05, 0.02) T[0,];
  slope1_raw ~ normal(0, 1);
  
  mu_slope2 ~ normal(0.05, 0.02);      
  sigma_slope2 ~ normal(0.03, 0.01) T[0,];  
  slope2_raw ~ normal(0, 1);
  ...
  for (i in 1:N) {
    ...
    for (t in 1:T) {
      real v_t = intercept[i] + slope1[i] * subtlex[i, t] + slope2[i] * greene[i, t];
      if (v_t <= 0)
        target += log(1e-10);
...
generated quantities {
  real log_lik[N];  // subject-level log-likelihood
